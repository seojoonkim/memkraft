...
_FINGERPRINT_EXCLUDED = frozenset({
    ...
    "anomaly_threshold_ms", "trigger_finish_seq", "trigger_elapsed_ms",
    "trigger_threshold_ms", "trigger_algorithm",
})
...
def _common(record_type: str, now: Any, authority_verified: bool,
    ...
    if authority_verified is not False:
        _fail("E_DELAY_VALIDATION", "authority_verified must be false",
              "authority_verified")
    return {
...
def _token(value: Any, pattern=_OPAQUE, limit: int = _MAX_TEXT) -> bool:
    return (isinstance(value, str) and 0 < len(value) <= limit
            and pattern.fullmatch(value) is not None)


def _persisted_valid(record: Dict[str, Any]) -> bool:
    """Strict semantic validation for untrusted persisted JSON."""
    kind = record.get("record_type")
    if kind == "delay_run_start":
        parent, refs = record.get("parent_run_id"), record.get("context_refs")
        return (_token(record.get("run_id"), _ID, 80) and record.get("kind") in _KINDS
                and _token(record.get("subject"))
                and (parent is None or _token(parent, _ID, 80))
                and isinstance(refs, list) and len(refs) <= _MAX_REFS
                and all(_token(ref, limit=_MAX_REF) for ref in refs))
    if kind == "delay_run_finish":
        elapsed, threshold = record.get("elapsed_ms"), record.get("anomaly_threshold_ms")
        return (_token(record.get("run_id"), _ID, 80)
                and isinstance(elapsed, int) and not isinstance(elapsed, bool) and elapsed >= 0
                and record.get("outcome") in _OUTCOMES and isinstance(record.get("anomaly"), bool)
                and (threshold is None or (isinstance(threshold, int)
                    and not isinstance(threshold, bool) and threshold >= 0)))
    fields = {"delay_retrospective": "observation", "delay_action": "action",
              "delay_application": "claim", "delay_verification": "finding"}
    if kind not in fields or not _token(record.get("chain_id"), _ID, 80):
        return False
    predecessor = record.get("predecessor_id")
    if not ((kind == "delay_retrospective" and predecessor is None)
            or (kind != "delay_retrospective" and _token(predecessor, _ID, 80))):
        return False
    if not _token(record.get(fields[kind])):
        return False
    if kind == "delay_verification" and record.get("verdict") not in _VERDICTS:
        return False
    if "external_receipt_ref" in record and not _token(record["external_receipt_ref"], limit=_MAX_REF):
        return False
    if kind == "delay_retrospective":
        seq, elapsed, threshold = (record.get("trigger_finish_seq"),
                                   record.get("trigger_elapsed_ms"),
                                   record.get("trigger_threshold_ms"))
        return (isinstance(seq, int) and not isinstance(seq, bool) and seq > 0
                and isinstance(elapsed, int) and not isinstance(elapsed, bool) and elapsed >= 0
                and isinstance(threshold, int) and not isinstance(threshold, bool) and threshold >= 0
                and record.get("trigger_algorithm") == "nearest-rank-p50-plus-4mad-v1")
    return True
...
def _estimate_runs(runs: Dict[str, Dict[str, Any]], kind: str, subject: str,
    ...
    p50, p80 = _nearest(values, .5), _nearest(values, .8)
    mad = _nearest([abs(value - p50) for value in values], .5)
...
def _fold(records: List[Dict[str, Any]]):
    ...
    for record in records:
        kind = record.get("record_type")
        if not _identity_fields_valid(record) or not _persisted_valid(record):
            corrupt += 1
            continue
        if kind == "delay_run_start":
            ...
            if not valid_parent:
                corrupt += 1
                continue
            if parent_id is not None and runs[parent_id]["finished"]:
                corrupt += 1
                continue
            runs[run_id] = {
                "kind": run_kind,
            ...
            threshold = estimate.get("anomaly_threshold_ms") if estimate.get("available") else None
            anomaly = bool(estimate.get("available") and threshold is not None
                           and elapsed > threshold)
            if (record.get("anomaly_threshold_ms") != threshold
                    or record.get("anomaly") is not anomaly):
                corrupt += 1
                continue
...
def _estimate_from(records: List[Dict[str, Any]], kind: str, subject: str,
    ...
    return _estimate_runs(runs, kind, subject, window, through_seq)
