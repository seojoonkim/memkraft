...
_EVENT_STORE = ".memkraft/project-sync/events.jsonl"
...
def inspect_project(path: str, fetch: bool = False) -> Dict[str, Any]:
    ...
    report: Dict[str, Any] = {
        "schema": "memkraft.version-drift/1",
        "checked_at": _now(),
        "path": str(repo),
        "exists": repo.exists(),
        "is_git": False,
...
def _event_path(base_dir: Optional[str]) -> Path:
    root = Path(base_dir).expanduser() if base_dir else Path(os.environ.get("MEMKRAFT_DIR", Path.cwd() / "memory"))
...
def record_event(report: Dict[str, Any], base_dir: Optional[str] = None, event: str = "scan") -> Dict[str, Any]:
    payload = dict(report)
    payload["event"] = event
    return store_core.append(_event_path(base_dir), payload)
