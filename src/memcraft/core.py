...
"""
    ...
    def _detect_regex(self, text: str) -> list:
        ...
        # 일본어 이름 패턴 (한자 2~4글자, 히라가나/카타카나는 제외)
        japanese_names = re.findall(r'[\u4E00-\u9FFF]{2,4}', text)  # 한자 기반, 중국어와 겹침 — 별도 처리
        korean_stopwords = set(self._load_stopwords().get("korean", []))
        chinese_stopwords = set(self._load_stopwords().get("chinese", []))
        japanese_stopwords = set(self._load_stopwords().get("japanese", []))
        chinese_stopwords = {'这个', '那个', '我们', '他们', '所以', '因为', '但是', '如果', '虽然', '可以', '需要', '应该', '已经', '正在', '开始', '完成', '进行', '结果', '问题', '方法', '开发', '应用', '设置', '确认', '更新', '添加', '修改', '删除', '创建', '运行', '测试', '分析', '搜索', '连接', '安装', '开源', '讨论', '报告', '启动', '实现', '使用', '系统', '功能', '数据', '项目', '公司', '技术', '通过', '关于', '之后', '之前', '今天', '明天', '昨天', '目前', '未来', '工作', '认为', '知道', '了解', '时候', '地方', '关系', '发展', '市场', '经济', '国家', '社会', '世界', '没有', '不是', '一样', '一些', '什么', '怎么', '为什么', '还是', '或者', '以及', '而且', '同时', '然后', '因此', '不过', '然而', '虽然', '只是', '只有', '已经', '正在', '将会', '可以', '应该', '必须', '可能', '能够', '希望', '觉得', '认为', '知道', '看到', '认为', '准备', '开始', '结束', '继续', '包括', '属于', '代表', '基于', '针对', '根据', '按照', '比较', '非常', '特别', '重要', '关键', '主要', '核心', '基本', '整个', '所有', '部分', '方面', '领域', '行业', '产业', '企业', '组织', '机构', '平台', '服务', '产品', '用户', '客户', '合作', '投资', '融资', '上市', '收购', '合并', '成立', '宣布', '发布', '推出', '上线', '下载', '注册', '登录', '加入', '参与', '提供', '支持', '帮助', '解决', '改善', '提升', '增加', '减少', '扩大', '缩小', '提高', '降低', '加快', '放慢', '推进', '推动', '促进', '阻碍', '影响', '改变', '创新', '突破', '领先', '竞争', '优势', '机会', '挑战', '风险', '问题', '方向', '目标', '计划', '策略', '方案', '措施', '步骤', '阶段', '过程', '结果', '效果', '影响', '价值', '意义', '作用', '地位', '角色', '责任', '任务', '内容', '形式', '结构', '体系', '机制', '模式', '特点', '特征', '属性', '类型', '种类', '范围', '规模', '水平', '程度', '速度', '频率', '周期', '趋势', '变化', '状态', '情况', '条件', '因素', '原因', '理由', '背景', '历史', '现状', '未来', '前景', '展望', '回顾', '总结', '分析', '评价', '判断', '选择', '决定', '建议', '意见', '观点', '看法', '想法', '思路', '方向', '路径', '方法', '手段', '工具', '技术', '标准', '规范', '规则', '原则', '理念', '概念', '定义', '分类', '标签', '版本', '模块', '组件', '接口', '参数', '配置', '选项', '属性', '方法', '事件', '对象', '实例', '类型'}
        # 일본어 불용어 (히라가나/카타카나 기반은 제외, 한자는 중국어와 공유)
        japanese_stopwords = {'私達', '彼等', '此れ', '其れ', '何れ', '但し', '然し', '又は', '及び', '且つ', '因みに', '処が', '然も', '乍ら', '為に', '依って', '就て', '対して', '就いて', '依り', '付き', '込み', '抜き', '通り', '毎に', '共', '等', '等', '達', '様', '殿', '氏', '君', 'さん', 'ちゃん', 'くん', 'せん', 'ません', 'でしょう', 'ましょう', 'ください', 'いただく', 'くださる', 'おる', 'ござる', 'なる', 'する', 'いる', 'ある', 'できる', 'わかる', '知る', '思う', '考える', '見る', '聞く', '読む', '書く', '話す', '言う', '行く', '来る', '帰る', '出る', '入る', '開く', '閉じる', '始める', '終わる', '続ける', '止める', '変わる', '増える', '減る', '上がる', '下がる', '作る', '作成', '実行', '確認', '設定', '更新', '追加', '削除', '修正', '分析', '検索', '接続', 'インストール', 'オープンソース', 'ディスカッション', 'レポート', 'システム', '機能', 'データ', 'プロジェクト', '会社', '技術'}
