...
def run_benchmark(*, repeats: int, seed: int, timeout: float, out: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="memkraft-reasoning-ab-") as tmp:
        ...
        for case, repeat in schedule:
            ...
            for order, condition in enumerate(conditions):
                prompt = build_prompt(case, hint if condition == "injected" else "")
                ...
                rows.append({
                    ...
                    "hint_chars": len(hint) if condition == "injected" else 0,
                    "hint_metadata": hint_meta if condition == "injected" else None,
                    "usage": usage,
...
def main() -> int:
    ...
    artifact = run_benchmark(repeats=args.repeats, seed=args.seed, timeout=args.timeout, out=args.out)
