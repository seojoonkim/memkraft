...
**MemCraft learns:** Auto-extraction from conversations is powerful. But Mem0 trades transparency (vector DB, opaque graph edges) for convenience.
**Absorbable ideas:** `memcraft extract` — auto-extract entities/facts from conversation text using regex + LLM, write directly to Markdown.
...
**MemCraft learns:** The ingest → process → query pipeline is elegant. Ontology grounding prevents knowledge drift.
**Absorbable ideas:** `memcraft cognify` — process raw inbox items into structured entity/decision pages. `memcraft search` — hybrid grep + semantic search.
...
**MemCraft learns:** Temporal knowledge graph is exactly what MemCraft's Timeline does, but Zep automates the "how facts change" tracking. This is a natural evolution for MemCraft.
...
**MemCraft learns:** Tiered memory is a powerful abstraction. MemCraft's Compiled Truth ≈ core, Timeline ≈ recall, originals/ ≈ archival. Making this explicit could improve UX.
**Absorbable ideas:** Explicit memory tier labeling. `memcraft promote` — move info from timeline to compiled truth.
...
**MemCraft learns:** Already adopted. Foundation of MemCraft's entity structure.
...
**MemCraft learns:** Already adopted. Live Notes + Meeting Brief come from Rowboat.
...
**What:** `memcraft extract "conversation text"` — auto-extract entities, facts, and relationships from any text.
...
**What:** `memcraft cognify` — process all inbox items into structured pages.
...
**What:** `memcraft search "query"` — combine grep (exact) with optional semantic search (vector).
...
**How:** Add tier field to templates. `memcraft promote "Entity" --tier core` to upgrade.
...
**How:** Track `[[links]]` in Markdown. `memcraft links "Entity"` shows graph.
**Difficulty:** Low-Medium | **Impact:** Medium | **Source:** Zettelkasten, Cognee

### Priority 7: Diff/Changelog
**What:** `memcraft diff` — show what changed since last run.
...
| # | Idea | Effort | Impact |
|---|------|--------|--------|
| 1 | `memcraft extract` — pipe text → auto-detect entities | 2h | High |
| 2 | `memcraft cognify` — process inbox → structured pages | 3h | High |
| 3 | Memory tier labels in templates | 30min | Medium |
| 4 | `memcraft diff` — show changes since last Dream Cycle | 1h | Medium |
| 5 | `memcraft search` — improve lookup with fuzzy matching | 2h | Medium |
| 6 | `memcraft links "Entity"` — show backlinks | 1h | Medium |
...
Transparency ◀─────────────────────────────────────▶ Automation
   MemCraft    GBrain    Cognee    Letta    Mem0    Zep
   (Markdown)  (MD)      (Graph)   (Tiered) (API)   (API)

                 ◀─────────────────────────────────────▶
                 Offline/Git-friendly    Cloud-required
                 MemCraft, GBrain        Mem0, Zep, Letta
```

**MemCraft's sweet spot:** Maximum transparency + reasonable automation. You can read, edit, git-track everything. Auto-extraction adds convenience without sacrificing readability.
