...
def _rewrite_record(path, index, **changes):
...
@pytest.mark.parametrize("changes", [
    {"subject": "raw subject"}, {"subject": 7}, {"context_refs": ["ref:x"] * 9},
])
def test_replay_strictly_validates_persisted_subject_and_refs(tmp_path, changes):
    mk = _mk(tmp_path)
    mk.delay_run_start("run-001", "task", "build", now=NOW)
    _rewrite_record(_path(mk), 0, **changes)
    assert mk.delay_estimate("task", "build", now=NOW)["reason"] == "corrupt_history"
    with pytest.raises(delay_ledger.DelayError) as excinfo:
        mk.delay_run_start("run-002", "task", "build", now=NOW)
    assert excinfo.value.code == "E_DELAY_LOG_CORRUPT"
...
@pytest.mark.parametrize("changes", [
    {"anomaly": "false"}, {"anomaly_threshold_ms": "10"}, {"anomaly": True},
])
def test_replay_reproduces_historical_anomaly_decision(tmp_path, changes):
    mk = _mk(tmp_path)
    _finish_sample(mk, 0, 10)
    _rewrite_record(_path(mk), 1, **changes)
    assert mk.delay_estimate("task", "build", now=NOW)["reason"] == "corrupt_history"


def test_replay_rejects_child_after_parent_finish(tmp_path):
    mk = _mk(tmp_path)
    mk.delay_run_start("task-001", "task", "build", now=NOW)
    mk.delay_run_finish("task-001", 10, now=NOW)
    records = [json.loads(line) for line in _path(mk).read_text().splitlines()]
    child = dict(records[0], id="child-record", event_seq=3, operation_id="child-op",
                 run_id="phase-001", kind="phase", subject="phase", parent_run_id="task-001")
    with _path(mk).open("a") as handle:
        handle.write(json.dumps(child) + "\n")
    assert mk.delay_estimate("task", "build", now=NOW)["reason"] == "corrupt_history"


def test_replay_rejects_parent_finish_with_unfinished_direct_child(tmp_path):
    mk = _mk(tmp_path)
    mk.delay_run_start("task-001", "task", "build", now=NOW)
    mk.delay_run_start("phase-001", "phase", "phase", now=NOW, parent_run_id="task-001")
    records = [json.loads(line) for line in _path(mk).read_text().splitlines()]
    finish = dict(records[0], id="finish-record", record_type="delay_run_finish",
                  event_seq=3, operation_id="finish-op", run_id="task-001", elapsed_ms=10,
                  outcome="completed", anomaly=False, anomaly_threshold_ms=None)
    for key in ("kind", "subject", "parent_run_id", "context_refs"):
        finish.pop(key, None)
    with _path(mk).open("a") as handle:
...
def test_strict_authority_bool_and_rejection_precedes_chain_state(tmp_path):
    mk = _mk(tmp_path)
    with pytest.raises(delay_ledger.DelayError) as excinfo:
        mk.delay_run_start("run-001", "task", "build", now=NOW, authority_verified=1)
    assert excinfo.value.code == "E_DELAY_VALIDATION"
    calls = (
...
def test_finish_and_chain_operation_id_mismatch_are_typed(tmp_path):
    mk = _mk(tmp_path)
    mk.delay_run_start("run-001", "task", "build", now=NOW)
    mk.delay_run_finish("run-001", 10, now=NOW, operation_id="shared-op")
    with pytest.raises(delay_ledger.DelayError) as excinfo:
        mk.delay_run_finish("run-001", 11, now=NOW, operation_id="shared-op")
    assert excinfo.value.code == "E_DELAY_IDEMPOTENCY_MISMATCH"
    for number in range(10, 15):
        _finish_sample(mk, number, 10, subject="other")
    anomalous = _finish_sample(mk, 99, 11, subject="other")
    mk.delay_record_retrospective("retro-001", anomalous["record"]["run_id"], "obs", now=NOW, operation_id="chain-op")
    with pytest.raises(delay_ledger.DelayError) as excinfo:
        mk.delay_record_retrospective("retro-001", anomalous["record"]["run_id"], "changed", now=NOW, operation_id="chain-op")
    assert excinfo.value.code == "E_DELAY_IDEMPOTENCY_MISMATCH"


def test_verdict_refs_duplicate_operation_and_all_public_methods(tmp_path):
    mk = _mk(tmp_path)
    ...
    assert mk.delay_estimate("task", "build", now=NOW)["reason"] == "corrupt_history"
    public = ("delay_run_start", "delay_run_finish", "delay_estimate",
              "delay_record_retrospective", "delay_record_action",
              "delay_record_application", "delay_record_verification")
    assert all(callable(getattr(mk, name)) for name in public)
